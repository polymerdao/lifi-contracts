...
usage() {
    echo "Usage: $0 <RPC_URL> <PRIVATE_KEY> <DESTINATION_DOMAIN> [OPTIONS]"
    echo ""
...
# Read TOKEN_MESSENGER and USDC from addresses.json if not set in env
...
echo -e "${BLUE}Calling PolymerCCTPFacet...${NC}"

# Run the call script
forge script ./script/deploy/facets/CallPolymerCCTPFacet.s.sol \
    --rpc-url "$RPC_URL" \
    --broadcast \
    --verify
